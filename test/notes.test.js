  ...
  describe.only('GET /api/notes', function () {
    ...
    it('should return correct search results for a title search', function () {
      ...
      const apiPromise = chai.request(app)
        .get(`/api/notes?searchTerm=${searchTerm}`).set('Authorization', `Bearer ${token}`);
      ...
      return Tag.findOne({userId: user.id})
        .then((_data) => {
          ...
          return Promise.all([
            ...
            chai.request(app).get(`/api/notes?tagId=${data.id}`).set('Authorization', `Bearer ${token}`)
          ]);
        })
        .then(([data, res]) => {
          expect(res).to.have.status(200);
    ...
    it('should return an empty array for an incorrect query', function () {
      ...
      const dbPromise = Note.find({
        ...
        userId: user.id}).sort({ updatedAt: 'desc' });
      const apiPromise = chai.request(app).get(`/api/notes?searchTerm=${searchTerm}`).set('Authorization', `Bearer ${token}`)
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
          expect(res).to.have.status(200);
    ...
    it('should update the note when provided a valid folderId', function () {
      ...
      return Promise.all([
        ...
        .then(([folder, note]) => {
          ...
          data = note;
          return chai.request(app).put(`/api/notes/${note.id}`).set('Authorization', `Bearer ${token}`).send(updateItem);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
          expect(res).to.be.json;
